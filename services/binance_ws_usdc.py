...
from config import BINANCE_WS_URL_USDC
...
def on_binance_message(ws, message):
    ...
    if data.get("e") == "markPriceUpdate":
        ...
        services.data_store.update_market_data("binance", "btc", "usdc", "mark_price",
                                               round(mark_price, 2), formatted_time)
        services.data_store.update_market_data("binance", "btc", "usdc", "funding_rate",
...
def start_binance_ws():
    ...
    while True:
        try:
            ws = websocket.WebSocketApp(BINANCE_WS_URL_USDC,
