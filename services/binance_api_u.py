...
from config import BINANCE_API_URL_U
...
def poll_binance_open_interest():
    ...
    while True:
        try:
            response = requests.get(BINANCE_API_URL_U)
            data = response.json()
            ...
            services.data_store.update_market_data("binance", "btc", "u", "open_interest", round(float(data["openInterest"]), 2), formatted_time)
